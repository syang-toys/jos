...
boot_alloc(uint32_t n)
{
	...
	//
	if (n > 0) {
		char *nnextfree = nextfree;
		nextfree = (char *)ROUNDUP((uint32_t)(n + nnextfree), PGSIZE);
		if (nnextfree > nextfree) {
...
page_init(void)
{
	...
	// free pages!
	size_t i;
	char *first_free_page_addr;
	size_t first_free_page_num;
	// 1)
	pages[0].pp_ref = 1;
	...
	// 4)
	first_free_page_addr = (char *)PADDR(boot_alloc(0));
	first_free_page_num = (size_t)first_free_page_addr/PGSIZE;
	for (i = EXTPHYSMEM/PGSIZE; i < first_free_page_num; i++) {
		pages[i].pp_ref = 1;
		pages[i].pp_link = NULL;
	}
	for (i = first_free_page_num; i < npages; i++) {
		pages[i].pp_ref = 0;
		pages[i].pp_link = page_free_list;
		page_free_list = &pages[i];
	}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for (addr = begin; addr < end; addr += PGSIZE) {
		...
		if (pte == NULL || (*pte & perm) != perm || addr >= ULIM) {
			user_mem_check_addr = begin == addr ? (intptr_t)va : addr;
			return -E_FAULT;			
		}
	}
	return 0;
